...
class GA_solve:
    ...
    def __mutate(self, gene):
        ...
        if random.random()<0.2:                  # 任意交换两点的顺序
    ...
    def fit(self):
        ...
        for epoch in range(num_epoch):
            ...
            index_alpha = list_index_sort[0]
            index_beta = list_index_sort[1]
            index_delta = list_index_sort[2]
            order_alpha = deepcopy(self.group[index_alpha][num_process:])
            order_beta = deepcopy(self.group[index_beta][num_process:])
            order_delta = deepcopy(self.group[index_delta][num_process:])
            a = 2 * (1-epoch/num_epoch)

            count = 0

            for i in range(3, num_group):
                order = deepcopy(self.group[list_index_sort[i]][num_process:])

                C1, C2, C3 = 2 * random.random(), 2 * random.random(), 2 * random.random()
                D_alpha = [C1 * order_alpha[_] - order[_] for _ in range(num_process)]
                D_beta = [C2 * order_beta[_] - order[_] for _ in range(num_process)]
                D_delta = [C3 * order_delta[_] - order[_] for _ in range(num_process)]

                A1, A2, A3 = a * (2 * random.random() - 1), a * (2 * random.random() - 1), a * (2 * random.random() - 1)
                X1 = [order_alpha[_] - A1 * D_alpha[_] for _ in range(num_process)]
                X2 = [order_beta[_] - A2 * D_beta[_] for _ in range(num_process)]
                X3 = [order_delta[_] - A3 * D_delta[_] for _ in range(num_process)]
                order = [(X1[_] + X2[_] + X3[_]) / 3 for _ in range(num_process)]
                order = list(np.array(order).argsort())

                #若order满足前后置关系，则print，看一下有多少满足的
                bool_print = False
                for i in range(num_process):
                    preprocess_id = self.preprocess[i]
                    if len(preprocess_id) != 0:  # 如果指针指向的元素有前置元素
                        indexesingene_pre = []
                        for j in range(len(preprocess_id)):
                            for index, value in enumerate(self.process):
                                if value == preprocess_id[j]:
                                    indexesingene_pre.append(order.index(index))  # 通过序号定位其在当前gene排序中的位置
                            if order.index(i) < max(indexesingene_pre):
                                bool_print = True
                if bool_print==True:
                    count += 1
            print(count)
            ...
            # 将子种群与父种群合并并去重
            self.group = self.group + self.new_group
            self.new_group = []
    ...
    def gante(self, gene):
        ...
        for i in range(num_process):
            plt.text(left[i], y[i], "工序%s" % process[color_list.index(color[i])], fontweight='heavy')
        # XY轴标签
        plt.xlabel("时间")
        for i in range(num_machine):
