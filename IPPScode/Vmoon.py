...
# 定义计算移动机器运行时间的函数，input_list为该机器的运行目标的序列

def truck_time(input_list):
    # 卡车的时间计算算法
    result = 0  
    return 0
def gantry_time(input_list):
    # 龙门吊的时间计算算法
    result = 0 
    return 0
def move_time(machine_num, input_list):
    if machine_num == 1:
         return truck_time(input_list)
    elif machine_num == 2:
        return gantry_time(input_list)
    else:
        return 0
# 定义移动机器类型及其对应的计算算法
move_machines = {
    ...
    def decoding(self, gene):
        ...
        for i in gene_order:
            ...
            if m in move_machines:
                ...
                move_machine_use[m-1].append(i)                # 将工序i记录在移动机器的使用列表中 
            gene_starttime[i] = machine_finishtime[m-1]    # 第m台机器的结束时间的位置为m-1
            ...
            if m in move_machines:
                gene_finishtime[i] = gene_starttime[i] + move_time(m,move_machine_use[m-1])  # 如果是移动机器则需要重新计算移动时间
            else:
